...
if __name__ == '__main__':
    ...
    parser.add_argument('--embedding_types', type=str, default='roberta',
    ...
    dataset_path = os.path.join(data_folder, 'Ebnerd_' + args['size'])
    ...
    def join_data(data_path):
        if "valid" in data_path:
            virality_path = os.path.join("./data", virality_version, "valid" + "_" + "virality" + ".csv")
        else:
            virality_path = os.path.join("./data", virality_version,
                                         data_path.split("/")[-2] + "_" + "virality" + ".csv")
        virality_df = pl.read_csv(virality_path)
        virality_df = virality_df.with_columns(pl.col("article_id").cast(pl.Int32))
        if "test2" in data_path:
            virality_df = virality_df.with_columns(pl.col("impression_id").cast(pl.UInt32))
            ...
            if args['neg_sampling']:
                ...
                if "test2" in data_path:
                    ...
                    sample_df = (
                        sample_df.rename({"article_id": "trigger_id"})
                        .collect()
                        .filter(pl.col("article_ids_clicked").apply(lambda x: len(x) == 1))
        ...
        sample_df = (
            ...
            .join(virality_df, on=["impression_id", "article_id"], how="left"))
        sample_df = (
            sample_df
            .with_columns(
    ...
    emb_path = dataset_path + f'/{embedding_types}_vector.parquet'
    emb_df = pl.read_parquet(emb_path)
    emb = pca.fit_transform(np.array(emb_df[emb_df.columns[-1]].to_list()))
    print(f"{embedding_types}_emb.shape", emb.shape)
    item_dict = {
        "key": emb_df["article_id"].cast(str),
        "value": emb
    }
    print(f"Save {embedding_types}_emb_dim{embedding_size}.npz...")
    np.savez(f"{data_folder}/{dataset_version}/{embedding_types}_emb_dim{embedding_size}.npz", **item_dict)
    ...
    np.savez(f"{data_folder}/{dataset_version}/inviews_{embedding_types}_emb_dim{embedding_size}.npz", **item_dict)
