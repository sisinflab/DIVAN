...
def extract_avg_auc(result_filename, expid):
    avg_auc_pattern = re.compile(r'avgAUC:\s*([\d\.]+)')
    with open(result_filename, 'r') as file:
        for line in file:
            if expid in line:
                match = avg_auc_pattern.search(line)
                if match:
                    return float(match.group(1))
    raise ValueError(f"avgAUC not found for experiment id {expid}")


def tpe_search(config_dir, gpu_list, script='run_expid.py', max_evals=20):
    experiment_id_list = load_experiment_ids(config_dir)
    gpu_list = list(gpu_list)
    idle_queue = list(range(len(gpu_list)))
    processes = dict()
    ...
    def objective(params):
        ...
        gpu_id = gpu_list[idle_idx]
        expid = params['expid']
        cmd = f"python -u {script} --config {config_dir} --expid {expid} --gpu {gpu_id}"
        print("\n Starting new trial...")
        ...
        # Read the performance metric from the result file
        avg_auc = extract_avg_auc(result_filename, expid)

        # Clean up
        idle_queue.append(idle_idx)

        return 1 - avg_auc  # Return 1 - avg_auc as we minimize the objective in Hyperopt

    trials = Trials()

    # Define the parameter space for Hyperopt
    space = {
        'expid': hp.choice('expid', experiment_id_list)
    }
