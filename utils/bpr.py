...
from utils.loss_utils import get_pairwise_diff_batch, get_truncated, masked_softmax
from utils.loss import ListWiseLoss


# BPR Loss as  described in orignial paper. https://arxiv.org/abs/1205.2618 This loss doesn't include regularization
# term as in tensorflow it should be done on the model side (e.g. include l2 regularization in embeddings) Setting
# softmax_weighted into True will turn this loss into BPR-max loss, as described in the GRU4Rec+ paper #
# https://dl.acm.org/doi/abs/10.1145/3269206.3271761
class BPRLoss(ListWiseLoss):
    ...
    def calc_per_list(self, y_pred, y_true):
        ...
        mask = tf.cast(
            (get_pairwise_diff_batch(top_true.values, true_ordered_by_pred, self.max_positives, pred_size) > 0),
            tf.float32)
        values = get_pairwise_diff_batch(pred_ordered_by_true, pred, self.max_positives, pred_size)
        sigmoid = -tf.math.log_sigmoid(values)
        sigmoid = sigmoid * mask
    ...
    def __call__(self, y_true, y_pred, reduction='mean'):
        result = self.calc_per_list(y_true, y_pred)
        if reduction == 'mean':
